...
import json
import json
print("JSON module loaded successfully!", json)
import os
print("File exists?", os.path.exists("symptom_data.json"))


# flask app
app = Flask(__name__)

#symptoms = ["itching", "cough", "fever", "headache", "fatigue"]
symptoms = [
...
def home():
    if request.method == 'POST':
        ...
        # Handle case where no symptoms are entered or invalid input
        if symptoms == "Symptoms" or not symptoms:
            ...
            user_symptoms = [symptom.strip("[]' ") for symptom in user_symptoms]  # Clean up input

            # Check if symptoms are valid and sufficient
            if len(user_symptoms) < 4 or any(symptom not in symptoms_dict for symptom in user_symptoms):
                message = "Consult a doctor.Insufficient symptoms were provided."
                return render_template('index.html', message=message)

            # Proceed with prediction if input is valid
...
@app.route('/symptom-info', methods=['POST'])
def get_symptom_info():
    ...
    for symptom in symptom_info:
        if symptom in user_input:
            response[symptom] = symptom_info[symptom]

    if not response:
        return jsonify({"message": "Sorry, I couldn't find any matching symptom info."})
